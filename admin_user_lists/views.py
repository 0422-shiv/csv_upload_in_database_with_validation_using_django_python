...
from my_uploads.serializer import DeploymentMetaDataFileDetailedDataSerializer, DeviceMetaDataFileDetailedDataSerializer, InputDataFileDetailedDataSerializer
from data_upload.serializer import UploadedCoAuthorSerializer
from data_upload.models import CsvFileData, CoAuthors
...
class UserListsView(LoggingMixin, generics.ListAPIView):
	permission_classes = (permissions.IsAdminUser,)
	pagination_class = CustomPageNumberPagination
	serializer_class = UserListsSerializer
	filter_backends = [filters.SearchFilter,filters.OrderingFilter]
	search_fields = ['username', 'email', 'institution_id__name', 'country', 'city']
	ordering_fields = ['username', 'email', 'institution_id__name', 'country', 'city']
...
class UserUploadsView(LoggingMixin, generics.ListAPIView):
	permission_classes = (permissions.IsAdminUser,)
	serializer_class = UserUploadsSerializer
	filter_backends = [filters.SearchFilter,filters.OrderingFilter]
	search_fields = ['assign_name','upload_identifier', 'created_at']
	ordering_fields =  ['assign_name','upload_identifier', 'created_at']
	def get_queryset(self):
		queryset = CsvFileData.objects.filter(created_by=self.kwargs['user_id']).filter(csv_type='Input Data').filter(
				fileuploadstatus_id=8)
		return queryset

	def get(self, request, user_id):
		self.filter_backends = (filters.SearchFilter,filters.OrderingFilter)
		self.search_fields = ['assign_name','upload_identifier', 'created_at']
		self.ordering_fields =  ['assign_name','upload_identifier', 'created_at']
		limit = self.request.query_params.get('limit', 10)
		user_details = UserListsSerializer(User.objects.filter(id=user_id), many=True).data
		...
		query=CsvFileData.objects.filter(created_by=instance).filter(csv_type='Input Data').filter(
						fileuploadstatus_id=8)
		user_uploads_count=query.count()
		upload_identifier_list=[] 
		for data in query:
				upload_identifier_list.append(data.upload_identifier)
		# total_species=20 #dummy data 
		...
		return Response({'status': True, 
						'user_details':user_details, 'user_upload': serializer.data,  
						...
						'total_records':paginator.page.paginator.count}, status=status.HTTP_200_OK)



class UserUploadsDetailsView(LoggingMixin, generics.ListAPIView):
	permission_classes = (permissions.IsAdminUser,)
...
class UserFileDetailedDataView(LoggingMixin,generics.ListAPIView):
	permission_classes = (permissions.IsAdminUser,)
	filter_backends = [filters.SearchFilter,filters.OrderingFilter]
	...
	def get(self,request,upload_identifier,csv_type):
		...
		if csv_type =='Deployment MetaData':
				self.search_fields =  ('accelerometry_calibrations_done', 'accelerometry_orientation_of_accelerometer_on_organism',
		...
		queryset = paginator.paginate_queryset(self.filter_queryset(self.get_queryset()),request )
		serializer = self.get_serializer(queryset, many=True) 
		upload_name=CsvFileData.objects.filter(upload_identifier=upload_identifier).filter(
				fileuploadstatus_id=8).filter(csv_type='Input Data').first()
		return Response({'status':True,
                   		'upload_name':upload_name.assign_name,
                     'user_name':upload_name.created_by.username,
						'next_page_link': paginator.get_next_link(),
						'previous_page_link': paginator.get_previous_link(),
						'record_per_page': int(limit),
      					'current_page': paginator.page.number,
